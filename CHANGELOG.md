...
- **backup.sh** also runs nightly at 4:00 AM via cron, after the dumps and cleanup so it captures the current night's dumps
- **backup.sh** rewritten: reads paths from `.env`, mirrors six defined sets with rsync to `BACKUP_DIR`, excludes media/downloads/artwork/caches, supports `--dry-run` and `--only`, keeps the `.env` copy at `chmod 600`, and no longer stops containers (it previously ran `docker-compose down`, used paths that do not exist, and needed `sudo`)
